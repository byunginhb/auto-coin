...
async function backtest(
  ...
  try {
    ...
    if (fs.existsSync(dataFilePath)) {
      console.log('Loading candlestick data from file...');
    ...
    for (let i = 50; i < candles.length; i++) {
      ...
      if (position === 'LONG') {
        ...
        if (profit >= takeProfitUSDT || profit <= stopLossUSDT) {
          // 롱 포지션 종료
    ...
    for (let takeProfit of takeProfitOptions) {
      for (let rsiBuy of rsiBuyThresholdOptions) {
        for (let rsiSell of rsiSellThresholdOptions) {
          console.log(
            `Testing with stopLoss: ${stopLoss}, takeProfit: ${takeProfit}, rsiBuy: ${rsiBuy}, rsiSell: ${rsiSell}`
          );
