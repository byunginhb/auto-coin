...
async function backtest(
  ...
  let results = [];
  let investmentFraction = 0.02; // 초기 투자 금액 비율 (5%)
  let currentInvestment = initialCapital * investmentFraction;
  ...
  try {
    ...
    for (let i = 50; i < candles.length; i++) {
      ...
      if (position === 'LONG') {
        const profit =
          (lastClose - entryPrice) * (currentInvestment / entryPrice);

        if (
          profit >= (takeProfitPercent / 100) * currentInvestment ||
          profit <= -(stopLossPercent / 100) * currentInvestment
        ) {
          // 롱 포지션 종료
          ...
          // 손실 발생 시 투자 금액을 2배로 증가
          if (profit < 0) {
            currentInvestment = Math.min(currentInvestment * 2, currentBalance);
          } else {
            // 이익 발생 시 초기 투자 금액으로 복원
            currentInvestment = initialCapital * investmentFraction;
          }
        ...
        const profit =
          (entryPrice - lastClose) * (currentInvestment / entryPrice);
    ...
    if (saveCSV) {
      ...
      const csvWriter = createCsvWriter({
        ...
        header: [
          ...
          { id: 'tradeAmount', title: 'TRADE_AMOUNT' },
        ],
      });
